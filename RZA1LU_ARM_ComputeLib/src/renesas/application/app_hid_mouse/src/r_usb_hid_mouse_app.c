...
static QueueHandle_t *g_usb_mouse_msg_q;
/******************************************************************************
Function Prototypes
******************************************************************************/
...
/******************************************************************************
* Function Name: hid_mouse_gui_if
* Description  : Test task to flash the LEDs
* Arguments    : none
* Return Value : none
******************************************************************************/
static void task_mouse_gui ( void * parms ) {
	usbM_t pMsg;
	bool_t button = false;
	int8_t x = 0;
	int8_t y = 0;

	QueueHandle_t * msg = g_usb_mouse_msg_q;
	if ( msg != NULL ) {
		while ( 1 ) {
			xQueueReceive ( msg, (void*) &pMsg, portMAX_DELAY);

			button = !pMsg.left_button;
			x = pMsg.xPos;
			y = pMsg.yPos;

			mse_send_in_report( button, x, y);
...
* Return Value : none
******************************************************************************/
int16_t gui_hid_mouse_init ( QueueHandle_t *msg )
{
    ...
    if((-1) != m_iusbfdevice)
    {
        ...
        if(0 == control(m_iusbfdevice, CTL_USBF_GET_CONFIGURATION, &config))
        {
            ...
            if(0 == control(m_iusbfdevice, CTL_USBF_SET_CONFIGURATION, &config))
            {
                ...
                if ( msg != NULL ) {
                	// Set Message Queue
					g_usb_mouse_msg_q = msg;

					// Start USB Task
					R_OS_CreateTask( "USBHID Mouse",
								task_mouse_gui,
								NULL,
