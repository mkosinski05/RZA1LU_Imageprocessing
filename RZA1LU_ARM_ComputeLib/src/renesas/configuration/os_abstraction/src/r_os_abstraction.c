...
static char *get_task_status (const char *task)
{
    ...
    for (i = 0, id = 0; i < ux_array_size; id++)
    {
        for (j = 0; j < ux_array_size; j++)
        {
            ...
            {
                return p_status_name[p_xt->eCurrentState];
            }
        }
    }
}
...
void R_OS_InitMemManager(void)
{
    ...
    xHeapRegions[0].xSizeInBytes = (MIRROR_HEAP_END - MIRROR_HEAP_START);
        ...
        int_t priority)
{
    ...
    /* Create a new task */
    status = xTaskCreate((pdTASK_CODE) task_code, name, (actual_stack / sizeof(StackType_t)), params, priority, &task);
...
int32_t R_OS_TaskGetPriority (uint32_t uiTaskID)
{
    ...
    {
        return (uint32_t) uxTaskPriorityGet((xTaskHandle) uiTaskID);
    }
    else
    {
        return -1;
