...
void R_SCI_UninitialiseHwIf(e_sc_id_t id)
{
    ...
    {
        /* Disable SCIF module */
        (*r_sci_device_config[id].cpg_reg) |= r_sci_device_config[id].cpg_stb_ctrl;
...
/* 1. Clear TE, RE, RIE, TIE and TEIE to 0 disable the serial port*/  // LLD
        ...
        /* Clear the FIFO */
        pPORT->SCFCR |= r_sci_device_config[res].scifx_scfcr_tfrst;    /* SCIFx_SCFCR_TFRST */
        pPORT->SCFCR |= r_sci_device_config[res].scifx_scfcr_rfrst;    /* SCIFx_SCFCR_RFRST */
        ...
        pPORT->SCSMR |= (uint16_t)byDividerClockCode;

/* 2. set the port configuration */ // HLD
        sciError |= sciConfigureSerialMode(pPORT, pConfiguration);

/* 3. set the baud rate */ // LLD
        pDDSCIF->dwActualBaud = sciSetBaudRate(&pPORT->SCBRR, pConfiguration->dwBaud,
...
/* 4. Enable RXD & TXD */ // LLD
        pPORT->SCSCR |= r_sci_device_config[res].scifx_scscr_te; /* SCIFx_SCSCR_TE */
        pPORT->SCSCR |= r_sci_device_config[res].scifx_scscr_re; /* SCIFx_SCSCR_RE */

        /* Enable receive interrupts */
        pPORT->SCSCR |= r_sci_device_config[res].scifx_scscr_rie; /* SCIFx_SCSCR_RIE */
...
{ // same as read
    ...
    pDDSCIF->pPORT->SCSCR |= r_sci_device_config[pDDSCIF->res].scif_scscr_tie; /* SCIFx_SCSCR_TIE */
}
/******************************************************************************
...
void sciINTTxi(PDDSCIF pDDSCIF)
{
    ...
    while (((pPORT->SCFDR&SCIF0_SCFDR_T) >> SCIF0_SCFDR_T_SHIFT) < 0x10)
    {
        ...
        if (!cbGet(pcBuffer, (uint8_t *) &pPORT->SCFTDR))
        {
            ...
            pPORT->SCSCR &= ~SCIF0_SCSCR_TIE;
            break;
        }
    }

    /* Clear the empty flag */
    pPORT->SCFSR &= ~SCIF0_SCFSR_TDFE;
}
/******************************************************************************
                              ...
                              _Bool  bfABCS)
{
    ...
    /* calculate baud rate */
    return ((dwClockFrequencyInHz / (byBRR + 1)) / wClockDivider);
}
/******************************************************************************
