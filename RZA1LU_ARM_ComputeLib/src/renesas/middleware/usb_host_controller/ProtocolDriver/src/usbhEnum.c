...
{ "Generic Hub Driver", no_dev_open, no_dev_close, no_dev_io, no_dev_io, no_dev_control, no_dev_io};
    ...
    else
    {
        ...
        && ((gUsbEnum.pPort->dwPortStatus & USBH_HUB_PORT_CONNECT_STATUS) == 0))
        {
            ...
            /* WARNING: This function can call free() when the device
             ...
             not an ISR!! */
            R_DEVLINK_DevRemove(gUsbEnum.pPort->pDevice->pszSymbolicLinkName);
...
static void enumDeviceRetryLoop (void)
{
    ...
    else
    {
        ...
        /* Link with a the device driver located or a NULL pointer if not
         supported */
        if (!R_DEVLINK_DevAdd(gUsbEnum.Information.pszDeviceLinkName, (PDEVICE *) get_no_driver(), &gUsbEnum.Information))
        {
...
static void enumDeviceComplete (void)
{
    ...
    static uint32_t uiDeviceID = 0;
    /* Fill out the device information */
    strcpy(gUsbEnum.Information.pszDeviceLinkName, pDevice->pszSymbolicLinkName);
    strcpy(gUsbEnum.Information.pszManufacturer, pDevice->pszManufacturer);
    strcpy(gUsbEnum.Information.pszProduct, pDevice->pszProduct);
    strcpy(gUsbEnum.Information.pszSerialNumber, pDevice->pszSerialNumber);
    ...
    sprintf(pDevice->pszSymbolicLinkName, "USB.VID0x%.4X.PID0x%.4X.I%u", pDevice->wVID, pDevice->wPID, uiDeviceID++);

    /* link with a the device driver located or a NULL pointer if not
     supported */
    if (!R_DEVLINK_DevAdd(pDevice->pszSymbolicLinkName, gUsbEnum.pDeviceDriver, &gUsbEnum.Information))
    {
...
//    devList(stdout);
    ...
    if (gUsbEnum.pDeviceDriver != &g_usb_hub_driver)
    {
        /* Temporary device is now finished with */
        gUsbEnum.pDevice = NULL;
        /* Set the device port pointer */
