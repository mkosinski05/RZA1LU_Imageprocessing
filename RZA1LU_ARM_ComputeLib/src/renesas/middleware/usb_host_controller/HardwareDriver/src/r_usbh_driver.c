...
#define USB_PIPESEL(p,n)            do {rza_io_reg_write_16(&(p)->PIPESEL, (n), USB_PIPESEL_PIPESEL_SHIFT, USB_PIPESEL_PIPESEL);}\
...
void R_USBH_Initialise (PUSB pUSB)
{
    ...
    if (0 == rza_io_reg_read_16(&SYSCFG0_0, USB_SYSCFG_UPLLE_SHIFT,  USB_SYSCFG_UPLLE))
    {
        ...
        usb0_prev_suspended = rza_io_reg_read_16(&SUSPMODE_0, USB_SUSPMODE_SUSPM_SHIFT,  USB_SUSPMODE_SUSPM);
        usb1_prev_suspended = rza_io_reg_read_16(&SUSPMODE_1, USB_SUSPMODE_SUSPM_SHIFT,  USB_SUSPMODE_SUSPM);
        ...
        uint8_t byDeviceAddress, uint8_t byEndpointNumber, uint8_t byInterval, uint16_t wPacketSize)
{
    ...
    switch (transferType)
    {
        case USBH_ISOCHRONOUS :
            ...
            rza_io_reg_write_16(&pUSB->PIPEPERI, ((byInterval - 1) & USB_PIPEPERI_IITV), USB_PIPEPERI_IITV_SHIFT, USB_PIPEPERI_IITV);
...
static void r_usbh_write_fifo (PUSB pUSB, void *pvSrc, size_t stLength)
{
    ...
    while (stWords--)
    {
        rza_io_reg_write_16(&pUSB->CFIFO, *pusSrc++, NO_SHIFT, GENERIC_16B_MASK);
    }
    /* If there are bytes */
    if (stBytes)
    {
        ...
        uint8_t *pbyDest = (uint8_t*)&pUSB->CFIFO;
        /* Select byte access */
        rza_io_reg_write_16(&pUSB->CFIFOSEL, 0, NO_SHIFT, USB_CFIFOSEL_MBW_0);
        /* Write the bytes */
        while (stBytes--)
        {
            *pbyDest = *pbySrc++;
...
static void r_usbh_read_fifo (PUSB pUSB, void *pvDest, size_t stLength)
{
    ...
    while (stLength--)
    {
        *pulDest++ = rza_io_reg_read_32((uint32_t *)&pUSB->CFIFO, USB_CFIFO_FIFOPORT_SHIFT, USB_CFIFO_FIFOPORT);
    }
    ...
    while (stLength--)
    {
        *pusDest++ = rza_io_reg_read_16(&pUSB->CFIFO, NO_SHIFT, GENERIC_16B_MASK);
    }
#endif
}
/******************************************************************************
