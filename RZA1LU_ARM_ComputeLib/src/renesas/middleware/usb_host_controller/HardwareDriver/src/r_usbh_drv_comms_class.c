...
static int_t drv_control(st_stream_ptr_t p_stream, CTLCODE ctl_code, void *p_ctl_struct);
...
static int_t drv_open(st_stream_ptr_t p_stream)
{
    /* Get a pointer to the device information from the host driver for this
     device */
    PUSBDI pdevice = usbhGetDevice (p_stream->p_stream_name);
    if (pdevice)
    {
    ...
    if (p_sci_drv)
    {
        ...
        if (bfresult)
        {
            ...
            if (p_sci_drv->write_request.errorCode)
            {
                ...
                /* Set the error code */
                if (REQ_IDLE_TIME_OUT == p_sci_drv->write_request.errorCode)
                {
    ...
    switch (ctl_code)
    {
        ...
        case CTL_SCI_GET_LINE_STATUS:
        {
            if (p_ctl_struct)
            {
                ...
                sci_lst.breakOutput = p_sci_drv->break_in_progress;
